...
const virtualizedScrollCoordinator = createVirtualizedScrollCoordinator({
  cancelFrame: handle => window.cancelAnimationFrame(handle),
...
function initializeVirtualizedDocumentWindow(): void {
  ...
  if (initialOperation !== null) {
    const controller = virtualizedDocumentWindowController;
    let pendingInitialVirtualizedWindowWork: ((operation: VirtualizedScrollOperation) => void) | null;
