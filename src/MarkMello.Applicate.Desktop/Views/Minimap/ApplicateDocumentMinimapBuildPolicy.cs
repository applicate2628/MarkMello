...
internal static class ApplicateDocumentMinimapBuildPolicy
{
    ...
    public static bool AllowsDetailedMiniature(ApplicateDocumentMiniatureSnapshot snapshot)
    ...
    private static bool HasSizeChanged(Size previous, Size current)
        => SysMath.Abs(previous.Width - current.Width) > LayoutSizeEpsilon
            || SysMath.Abs(previous.Height - current.Height) > LayoutSizeEpsilon;
}
